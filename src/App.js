...
// TODO: refactor, finish styling, edit todo
  ...
  return (
    <div>
      <div className={`${showModal ? 'block' : 'hidden'}`}>
        <Modal todo={currentTodo} sendEdit={sendEdit} toggleModal={toggleShowModal}/>
      </div>
      <form onSubmit={(e)=>{addTodo(e)}} className="h-24 bg-green-500 rounded-b-lg w-screen items-center justify-between flex">
        <input className="h-12 w-3/4 mx-12 bg-transparent text-white placeholder-white outline-none text-2xl" placeholder="Enter Something..." 
      ...
      <ul className="m-4">
        {todolist.map((todo) => (
            <li className="m-2 h-16 rounded-lg bg-gray-200 flex justify-between items-center" key={todo.todo_id}>
            <span className="mx-12 text-2xl">{todo.description}</span>
            <div>
              {/* EDIT button */}
              <button className="m-2"
                onClick={()=> editTodo(todo)}>
                  <svg className="h-8 w-8 stroke-current text-yellow-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              onClick={()=> completeTodo(todo)}>
               ...
               {/* DELETE button */}
              <button className=" h-8 m-2 mr-8 stroke-current text-red-500" onClick={()=>{deleteTodo(todo.todo_id)}}>
      ...
      <div className={`${showCompleted ? 'block h-auto' : 'hidden h-0'}`}>
        ...
        <ul className="m-4">
          {completedTodos.map((todo) => (
            <li className="m-2 h-16 rounded-lg bg-gray-200 flex justify-between items-center" key={todo.id}>
              <span className="mx-12 text-2xl">{todo.description}</span>
              {/* DELETE button */}
              <button className=" h-8 m-2 mr-8 stroke-current text-red-500" onClick={()=>{deleteCompleted(todo.id)}}>
                <svg className="h-8 w-8" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
              </button>
